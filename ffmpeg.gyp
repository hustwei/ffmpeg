...

{
  ...
  'variables': {
    # Nesting these variable definitions in another variables dict lets us use
    # them to set platform_config_root below.
    'variables': {
      'conditions': [
    ...
    # platform_config_root depends on the value of os_config, however we can't
    # define os_config in the nested variable dict above: its value
    # depends on 'msan', but msan is set in common.gypi in the second level variables
    # dict. That means 'msan' is undefined in the second level variables dict above.
    'conditions': [
      ['OS == "mac" or OS == "win" or OS == "openbsd" or OS == "android"', {
        'os_config%': '<(OS)',
        'platform_config_root': 'chromium/config/<(ffmpeg_branding)/<(OS)/<(ffmpeg_config)',
      }, {  # all other Unix OS's use the linux config
        'conditions': [
          ['msan==1', {
            # MemorySanitizer doesn't like assembly code.
            'os_config%': 'linux-noasm',
            'platform_config_root': 'chromium/config/<(ffmpeg_branding)/linux-noasm/<(ffmpeg_config)',
          }, {
            'os_config%': 'linux',
            'platform_config_root': 'chromium/config/<(ffmpeg_branding)/linux/<(ffmpeg_config)',
          }]
        ],
      }],
    ],
